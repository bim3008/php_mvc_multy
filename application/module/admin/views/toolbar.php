<?php
     ...
     $lblFilterGroupId   = isset($this->arrParam['filter_group_id']) ? $this->arrParam['filter_group_id'] : '';
     // ADD
     ...
     //ALL
     $linkAll         = URL::createLink($lblModule , $lblController, 'index', array('filter_status' => '' , 'filter_search' =>  $lblFilterSearch , 'filter_group_id' => $lblFilterGroupId));
     $btnAll          = HelperAdmin::cmsButton($linkAll, 'btn btn-info', '', 'all', 'All' . '(' . $this->countItems[0]['total'] . ')');
     // ACTIVE
     $linkActive      = URL::createLink($lblModule , $lblController, 'index', array('filter_status' => 'active', 'filter_search' => $lblFilterSearch, 'filter_group_id' => $lblFilterGroupId ));
     $btnActive       = HelperAdmin::cmsButton($linkActive, 'btn  bg-warning', '', 'ac', 'Active' . '(' . $this->countItems[1]['total'] . ')');
     // INACTIVE
     $linkInactive    = URL::createLink($lblModule , $lblController, 'index', array('filter_status' => 'inactive', 'filter_search' => $lblFilterSearch,'filter_group_id' => $lblFilterGroupId));
